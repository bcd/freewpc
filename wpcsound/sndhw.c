...
{
	return *(volatile U8 *)WPCS_CPU_INPUT;
}

extern inline void cpu_latch_write (U8 val)
{
	*(volatile U8 *)WPCS_CPU_OUTPUT = val;
}

extern inline void dac_write (U8 val)
{
	*(volatile U8 *)WPCS_DAC_DATA = val;
}


extern inline void adjust_volume (const U8 up_down_flag)
{
	*(volatile U8 *)WPCS_VOLUME = (up_down_flag << 1) | 1;
	*(volatile U8 *)WPCS_VOLUME = (up_down_flag << 1) | 0;
}
...
void wpc_sound_init (void)
{
	...
	for (i=0; i < sizeof (dac_beep); i++)
	{
		dac_write (dac_beep[i]);
		mdelay (1);
	}
}
