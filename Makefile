...
ifneq ($(nodefs),y)
MAKE_DEPS = Makefile $(MACHINE_DIR)/Makefile
endif
MAKE_DEPS += .config
...
$(PAGED_LINKCMD) : $(MAKE_DEPS) $(BLDDIR)/Makefile.xbms platform/$(PLATFORM)/Makefile
...
#
$(LINKCMD) : $(MAKE_DEPS) $(BLDDIR)/Makefile.xbms platform/$(PLATFORM)/Makefile
	$(Q)echo Creating linker command file $@ ... ;\
	rm -f $(LINKCMD) ;\
	echo "-mxswz" >> $(LINKCMD) ;\
...
CONFIG_FILES = $(BLDDIR)/mach-config.h $(CONFIG_SRCS)

.PHONY : config
config : $(CONFIG_FILES)

$(BLDDIR)/mach-Makefile : $(MACH_DESC)
...
HOST_XBM_LIBS = $(BLDDIR)/pgmlib.o

HOST_XBM_OBJS = $(BLDDIR)/sysgen.o $(BLDDIR)/borders.o $(BLDDIR)/backgrounds.o

HOST_XBM_CFLAGS = -Itools/pgmlib -Iinclude -g

xbmgen_clean :
	$(Q)echo "Cleaning host XBM files..." && \
	rm -f $(BLDDIR)/xbmgen $(HOST_XBM_LIBS) $(HOST_XBM_OBJS)

.PHONY : xbmgen_run
xbmgen_run : $(BLDDIR)/xbmgen
ifeq ($(PLATFORM),wpc)
	$(Q)$(MAKE) xbmgen_objs
endif
...
$(HOST_XBM_OBJS) : $(BLDDIR)/%.o : images/%.c
	$(HOSTCC) -o $@ -c $< $(HOST_XBM_CFLAGS)

$(BLDDIR)/pgmlib.o : tools/pgmlib/pgmlib.c
	$(HOSTCC) -o $@ -c $< $(HOST_XBM_CFLAGS)
